...
function handlePurchaseError(e: any): void {
  ...
  switch (code) {
    ...
    case PURCHASES_ERROR_CODE.CONFIGURATION_ERROR:
      Alert.alert('Configuration Error', 'There is a configuration issue. Please contact support.');
      break;
...
export function useRevenueCat() {
  ...
  const presentPaywall = async (): Promise<boolean> => {
    try {
      ...
      return (
        result === RevCatUI.PAYWALL_RESULT.PURCHASED || result === RevCatUI.PAYWALL_RESULT.RESTORED
      );
    } catch (e) {
