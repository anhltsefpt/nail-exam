...
export interface QuizState {
  ...
  firstTryMistakeIds: string[]; // question IDs answered wrong on the FIRST try
...
export const useQuizStore = create<QuizState>()((set, get) => ({
  ...
  submitAnswer: () => {
    ...
    set((state) => {
      const isFirstAttempt = !state.masteredIds.includes(question.id) && !state.firstTryMistakeIds.includes(question.id);
      ...
      const newRoundMistakes = !isCorrect
        ? (state.roundMistakes.includes(question.id) ? state.roundMistakes : [...state.roundMistakes, question.id])
        : state.roundMistakes.filter(id => id !== question.id);

      const newFirstTryMistakes = (!isCorrect && isFirstAttempt)
        ? [...state.firstTryMistakeIds, question.id]
        : state.firstTryMistakeIds;
      ...
      return {
        ...
        firstTryMistakeIds: newFirstTryMistakes,
      };
    });
  ...
  nextQuestion: () => {
    const { currentIndex, activeQuestions, roundMistakes } = get();
