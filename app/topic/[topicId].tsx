...
export default function TopicDetailScreen() {
  ...
  // --- Flash animation for returning from result screen ---
  const flashSetIndex = useUserStore((s) => s.flashSetIndex);
  const setFlashSetIndex = useUserStore((s) => s.setFlashSetIndex);
  const flashScale = useSharedValue(1);

  useEffect(() => {
    if (flashSetIndex !== null) {
      flashScale.value = withRepeat(
        withSequence(
          withTiming(0.96, { duration: 300, easing: Easing.inOut(Easing.ease) }),
          withTiming(1.04, { duration: 300, easing: Easing.inOut(Easing.ease) }),
      ...
      const timer = setTimeout(() => {
        flashScale.value = withTiming(1, { duration: 300 });
        setFlashSetIndex(null);
      }, 3000);
      ...
      >
        <SafeAreaView edges={['top']} style={styles.headerInner}>
          ...
          >
            ...
            <Typography variant="body" weight="semibold" style={{ color: 'white', marginLeft: 4 }}>
              {t('topic.roadmap')}
            </Typography>
          </TouchableOpacity>
          ...
          {!loading && (
            <Typography variant="caption" style={{ color: 'rgba(255,255,255,0.75)', marginTop: 4 }}>
              {t('topic.setsInfo', {
                ...
                questions:
                  sets.length > 0
                    ? t('topic.questionsEach', { count: sets[0].count })
                    : t('topic.zeroQ'),
              })}
        ...
        {loading ? (
          ...
          <>
            {sets.map((set, index) => {
              ...
              // Pro users: all sets accessible
              // Free users: only set 1 (index 0) is accessible
              const isUnlocked = isPro ? true : index === 0;
              const isActive = isPro
                ? !isPassed &&
                  index === sets.findIndex((_, i) => (topicProgress[i] || 0) < PASS_THRESHOLD)
                : index === unlockedSetIndex && !topicComplete;

              const isFlashing = flashSetIndex === index;

              return (
                <Animated.View key={index} style={isFlashing ? flashStyle : undefined}>
                  <TouchableOpacity
                    style={[
                      styles.setCard,
                    {
                      borderColor: isPassed ? theme.success : isActive ? phaseColor : theme.border,
                ...
                >
                  ...
                  <View style={styles.setInfo}>
                    ...
                    <Typography
                      variant="caption"
                      style={{
                        color: isUnlocked ? theme.textSecondary : theme.textMuted,
                      }}
                    >
                  ...
                  {isUnlocked && <ChevronRight size={20} color={theme.textMuted} />}
                </TouchableOpacity>
              </Animated.View>
              );
            })}
